...
export function HomeScreen() {
  ...
  if (query.isError || !query.data) {
    return (
      <ErrorView
        label="Impossible de charger l'accueil."
        ...
        <>
          ...
          <View style={styles.metrics}>
            <MetricCard
              ...
              value={query.data.summary.completedToday}
              tone="success"
            />
            <MetricCard
              label="En retard"
      ...
      renderItem={({item}) => (
        <View style={styles.activity}>
          <Text style={styles.activityTitle}>
            {item.agent.name} · {item.checkpoint.name}
          </Text>
          <Text style={styles.meta}>
            {item.round.name} · {item.site.name}
          </Text>
          <Text style={styles.meta}>
            {formatDateTime(item.scannedAt ?? item.scan?.scannedAt)}
          </Text>
          <Text
            style={[
              styles.order,
              {color: item.isOutOfOrder ? colors.warning : colors.success},
            ]}>
...
const styles = StyleSheet.create({
  ...
  order: {marginTop: 5, fontWeight: '800'},
});
